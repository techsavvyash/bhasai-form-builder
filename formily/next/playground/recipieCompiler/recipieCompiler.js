...
let RecipeCompiler = class {
  ...
  createLogicGraph(flowLogic, nodeIndexMap) {
    const logicGraph = Array.from(
      { length: flowLogic.nodes.length },
      () => /* @__PURE__ */ new Set()
    )
    flowLogic.edges.forEach((edge) => {
      if (
        nodeIndexMap[edge.source] == void 0 ||
        nodeIndexMap[edge.target] == void 0
      ) {
  ...
  traverse(node, logicGraph, indexNodeMap, edgeMapping, visited, logicDef) {
    ...
    const config = {}
    const configParameters = new Set(
      indexNodeMap[node].data.inputParams.map((input) => input.name)
    )
  ...
  compileLogic(flowLogic) {
    ...
    logicDef.transformers.sort(
      (t1, t2) => nodeIndexMap[t1.id] - nodeIndexMap[t2.id]
    )
    return logicDef
  }
}
export { RecipeCompiler }
